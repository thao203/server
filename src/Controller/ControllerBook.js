        ...
        try {
            ...
            if (Array.isArray(vitri)) {
                updatedVitri = vitri.map(item => {
                    ...
                    // Kiểm tra ràng buộc: soluong mới không được nhỏ hơn soluongmuon
                    if (newSoluong < currentSoluongmuon) {
                        throw new Error(`Số lượng tại vị trí ${item.mavitri} không hợp lệ: số lượng mới (${newSoluong}) nhỏ hơn số lượng đang mượn (${currentSoluongmuon})!`);
                    }
